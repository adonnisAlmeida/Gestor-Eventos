odoo.define('website_event_track_uclv.contentshare', function (require) {
'use strict';

const dom = require('web.dom');

$.fn.sharetrack = function (options) {
    
    var option = $.extend($.fn.sharetrack.defaults, options);
    var selected_text = "";
    $.extend($.fn.sharetrack, {
        init: function (shareable) {
            var self = this;            
            $.fn.sharetrack.defaults.shareable = shareable;
            $.fn.sharetrack.defaults.shareable.on('mouseup', function () {
                if ($(this).parents('body.editor_enable').length === 0) {
                    self.popOver();
                }
            });
            $.fn.sharetrack.defaults.shareable.on('mousedown', function () {
                self.destroy();
            });
        },
        getContent: function () {
    ...
    $.fn.sharetrack.init(this);
};

$.fn.sharetrack.defaults = {
